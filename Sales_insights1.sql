...
SELECT sum(transactions.sales_amount) as Total_Sales , MAX(transactions.sales_qty) as Maximum_QTY , 
markets.markets_name , transactions.market_code
FROM transactions 
INNER JOIN markets 
ON transactions.market_code = markets.markets_code
Group by markets.markets_name 
order by MAX(transactions.sales_qty) DESC
Limit 1
...
# 14. Top 3 Cities with highest sales 
SELECT markets.markets_name , sum(transactions.sales_amount) 
From transactions
inner join markets
on transactions.market_code = markets.markets_code
Group by markets.markets_name 
